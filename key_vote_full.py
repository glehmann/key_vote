...
def generateCandidate( candidateSize, hands, wrong=[] ):
  candidate = []
  nbOfKeys =  len(hands[0]) + len(hands[1])
  for i in range( candidateSize ):
    n = random.randint( 0, nbOfKeys - 1 )
    if n >= len( hands[0] ) :
      n = n - len( hands[0] ) + 100
  ...
  if candidate == list( wrong ):
    return generateCandidate( candidateSize, hands, wrong )
...
#    for min, max in [ ( 0, len( leftHand ) - 1 ) ] * nbOfVotes + [ ( 100, 100 + len( rightHand ) - 1 ) ] * nbOfVotes:
      candidate1 = generateCandidate( nbOfChars, hands )
      candidate2 = generateCandidate( nbOfChars, hands, candidate1 )
